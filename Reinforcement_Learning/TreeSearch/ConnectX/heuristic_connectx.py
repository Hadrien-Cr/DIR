...
def heuristic_reward_connectx(env, state):
...
def heuristic_sort_connectx(env, last_state,l,bit_player):
    l=sorted(l,key=(lambda action: abs(action[1]-env.dim_col/2)))
    return l

def random_policy_connectx(env,current_state,bit_player):
    return(choice(env.get_available_actions(current_state,bit_player)))

def policy_connectx_with_heuristic_reward(env,current_state,bit_player):
    actions=env.get_available_actions(current_state,bit_player)
    return(max(actions, key = lambda action: heuristic_reward_connectx(env, env.get_next_state(current_state,action))))