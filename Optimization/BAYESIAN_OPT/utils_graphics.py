...
def plot_1d(X, y, model, size, objective_fun, bounds):
	""" plot real observations vs surrogate function """
	# scatter plot of inputs and real objective function
	plt.scatter(X[size:], y[size:], marker='^',c="orange",label='last samples')
	plt.scatter(X[:size], y[:size],c="brown",label='initial samples')


	# line plot of surrogate function across domain5
	Xsamples = asarray(arange(bounds[0][0], bounds[0][1], 0.001))
...
def plot_2d(X, y, model, size, objective_fun, bounds):
	"""Plot real observations vs surrogate function in 2D."""
	# Scatter plot of inputs and real objective function
	fig = plt.figure()
	ax = fig.add_subplot(111, projection='3d')

	ax.scatter(X[size:, 0], X[size:, 1], y[size:], marker='^', c='orange', label='Last samples')
	ax.scatter(X[:size, 0], X[:size, 1], y[:size], c='brown', label='Initial samples')
